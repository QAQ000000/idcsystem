...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-semibold">编辑优惠券</h1>
    </div>
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="2" maxlength="500"
                              class="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400">{{ old('description', $coupon->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="type" x-model="type"
                            class="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400">
                        <option value="fixed" {{ old('type', $coupon->type) === 'fixed' ? 'selected' : '' }}>固定金额减免</option>
                        <option value="percent" {{ old('type', $coupon->type) === 'percent' ? 'selected' : '' }}>百分比折扣</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="stock" value="{{ old('stock', $coupon->stock) }}"
                           min="0" step="1"
                           class="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="product_ids"
                           value="{{ old('product_ids', $coupon->product_ids ? implode(',', $coupon->product_ids) : '') }}"
                           placeholder="例：1,2,5"
                           class="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400">
                </div>
    ...
    <script>
        function couponForm() {
            return { type: '{{ old('type', $coupon->type) }}' };
        }
    </script>
@endsection
