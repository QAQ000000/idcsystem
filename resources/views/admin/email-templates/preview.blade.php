...
@section('title', '预览邮件模板')

@section('content')
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold">预览邮件模板</h1>
            <p class="mt-1 text-sm text-slate-500">{{ $emailTemplate->name }}</p>
        </div>
        <div class="flex gap-2">
            <a class="rounded border px-4 py-2 text-sm" href="{{ route('admin.email-templates.edit', $emailTemplate) }}">编辑模板</a>
    ...
    <div class="grid gap-6 lg:grid-cols-[1fr_360px]">
        <section class="rounded bg-white p-6 shadow-sm">
            <div class="text-sm text-slate-500">邮件主题</div>
            <h2 class="mt-1 text-xl font-semibold">{{ $renderedSubject }}</h2>
            <div class="mt-6 rounded border bg-slate-50 p-5 text-sm leading-7">
                {!! $renderedBody !!}
            </div>
        </section>

        <aside class="space-y-6">
            <form method="post" action="{{ route('admin.email-templates.test', $emailTemplate) }}" class="rounded bg-white p-5 text-sm shadow-sm">
                @csrf
                <h2 class="mb-4 font-semibold">发送测试邮件</h2>
                <label class="block">
                    收件邮箱
                    <input class="mt-1 w-full rounded border px-3 py-2" name="email" type="email" required>
                </label>
                @if ($errors->any())
