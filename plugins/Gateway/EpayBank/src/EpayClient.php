...
class EpayClient
{
    public function __construct(private string $apiUrl, private string $pid, private string $key)
    {
    ...
    {
        return $this->request($this->apiUrl . 'api.php?act=order', ['pid' => $this->pid, 'key' => $this->key, 'trade_no' => $tradeNo]);
    }
    ...
    private function request(string $url, array $post = []): array
    {
        ...
        return $parsed ?: ['code' => $response->successful() ? 1 : -1, 'raw' => $response->body()];
    }
