...
class EpayAlipayPlugin extends AbstractPlugin implements PaymentGatewayInterface
{
    private const PAY_TYPE = 'alipay';
    private const NAME = 'epay_alipay';
    private const TITLE = '易支付 - 支付宝';
    private const DESCRIPTION = '通过彩虹易支付聚合平台收取支付宝付款';
    ...
    private function client(array $config): EpayClient
    {
        return new EpayClient($config['api_url'], $config['pid'], $config['key']);
    }

    private function notifyUrl(): string
    {
        return Route::has('plugin.epay_alipay.notify')
            ? route('plugin.epay_alipay.notify')
            : url('/plugin/epay_alipay/notify');
    }
    ...
    private function epayPayload(array $data): array
    {
    ...
    private function validatedConfig(): array
    {
        ...
        foreach (['api_url', 'pid', 'key'] as $key) {
